...
CREATE OR REPLACE TABLE jour_special (
    id INT AUTO_INCREMENT PRIMARY KEY,
    date DATE NOT NULL,
    type ENUM('dimanche', 'ferrie') NOT NULL
);

INSERT INTO jour_special (date, type) VALUES('2025-07-13', 'dimanche');
INSERT INTO jour_special (date, type) VALUES('2025-07-20', 'dimanche');
INSERT INTO jour_special (date, type) VALUES('2025-07-27', 'dimanche');
INSERT INTO jour_special (date, type) VALUES('2025-08-25', 'dimanche');
INSERT INTO jour_special (date, type) VALUES('2025-08-10', 'dimanche');
INSERT INTO jour_special (date, type) VALUES('2025-08-17', 'dimanche');
...
('Amine Bensaïd', 'ETU001', '2000-01-01', 'ETU001', '', '0123456789', 2),
('Sarah El Khattabi', 'ETU002', '2000-01-01', 'ETU002', '', '0123456789', 2),
('Youssef Moujahid', 'ETU003', '2000-01-01', 'ETU003', '', '0123456789', 2),
('Nadia Benali', 'ENS001', '2000-01-01', 'ENS001', '', '0123456789', 2),
('Karim Haddadi', 'ENS002', '2000-01-01', 'ENS002', '', '0123456789', 2),
('Salima Touhami', 'ENS003', '2000-01-01', 'ENS003', '', '0123456789', 2),
('Rachid El Mansouri', 'PROF001', '2000-01-01', 'PROF001', '', '0123456789', 2),
('Amina Zerouali', 'PROF002', '2000-01-01', 'PROF002', '', '0123456789', 2),
...
(1, 2),  -- Etudiant (assuming ID 1) can borrow 5 books
(2, 3), -- Professeur (assuming ID 2) can borrow 10 books
(3, 4),  -- Professionnel (assuming ID 3) can borrow 7 books

(4, 1);   -- Anonyme (assuming ID 4) can borrow 2 books
...
('Les Miserables', 'Victor Hugo', 1, '9782070409189', 'Gallimard', 96, 'Description'),
('L Etranger', 'Albert Camus', 2, '9782070360022', 'Gallimard1', 96, 'Description'),
...
('Harry Potter a l ecole des sorciers', 'J.K. Rowling', 3, '9782070643026', 'Pocket', 1216, 'Description');
...
-- INSERT INTO etat_reservation (id_reservation,
    ...
    SELECT 
        ...
        CASE 
            ...
            WHEN tp.id = 1 THEN
                CASE 
                    WHEN ta.id = 1 THEN 7  -- Etudiant: 14 jours
                    WHEN ta.id = 2 THEN 9  -- Professeur: 21 jours
                    WHEN ta.id = 3 THEN 12  -- Professionnel: 14 jours
                    WHEN ta.id = 4 THEN 2   -- Anonyme: 7 jours
                    ELSE 2  -- Défaut: 14 jours
                END
        END as nb_jour_pret,
        
        -- Pénalité par jour de retard selon le type d'adhérent et le type de prêt
        CASE 
            WHEN ta.id = 1 THEN 10  -- Etudiant: 2 jours de pénalité
            WHEN ta.id = 2 THEN 9  -- Professeur: 1 jour de pénalité
            WHEN ta.id = 3 THEN 8  -- Professionnel: 2 jours de pénalité
            WHEN ta.id = 4 THEN 20  -- Anonyme: 5 jours de pénalité
            ELSE 20  -- Défaut: 2 jours
        ...
        CASE 
            ...
            WHEN ta.id = 3 THEN 7  -- Professionnel: 3 jours avant
            WHEN ta.id = 4 THEN 1  -- Anonyme: 2 jours avant
            ELSE 1  -- Défaut: 3 jours
        END as nb_jours_avant_prolongation,
        
        -- Nombre de jours de prolongation accordée
        CASE 
        -- Pas de prolongation pour les prêts "Sur place"
            WHEN ta.id = 1 THEN 3   -- Etudiant: 7 jours de prolongation
            WHEN ta.id = 2 THEN 5  -- Professeur: 14 jours de prolongation
            WHEN ta.id = 3 THEN 7   -- Professionnel: 7 jours de prolongation
            WHEN ta.id = 4 THEN 2   -- Anonyme: 3 jours de prolongation
